...
void main() {
  test(
    'successful Flutter exit means finish only while the same app connection remains',
    () {
      expect(
  ...
  test(
    'wrapper preserves async and argument-taking main without installing a binding or UI',
    () {
  ...
  test(
    'instrumentation preserves app files and release manifest; live lib and path dependencies stay consistent',
    () async {
      final temp = await Directory.systemTemp.createTemp(
        'airreload-instrument-',
      );
      try {
        ...
        expect(
          await File(
            p.join(p.dirname(prepared.target), 'runtime.dart'),
          ).readAsString(),
          allOf(
        ...
        expect(
          await File(
            p.join(p.dirname(prepared.target), 'tunnel.dart'),
          ).exists(),
          isFalse,
        );
        final document =
            loadYaml(
                  await File(
                    p.join(prepared.directory, 'pubspec.yaml'),
                  ).readAsString(),
                )
                as YamlMap;
        expect(
        ...
        expect(
          await File(
            p.join(prepared.directory, 'lib', 'main.dart'),
          ).readAsString(),
          contains('changed'),
        );
  ...
  test(
    'session TLS host authenticates, reports an app, and allows reconnect after disconnect',
    () async {
      final temp = await Directory.systemTemp.createTemp(
  ...
  test(
    'pairing endpoint accepts one ABI report and discloses an APK only when ready',
    () async {
